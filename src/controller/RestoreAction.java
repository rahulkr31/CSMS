...
@WebServlet("/restore")
public class RestoreAction extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(page.equals("area"))
		{
			try
			{
				...
				if(r >= 1) {
					request.setAttribute("notify", "Area Restored.");
					rd = request.getRequestDispatcher("restorearea.jsp");
				}
		...
		else if(page.equals("pack"))
		{
			try
			{
				...
				if(r >= 1) {
					request.setAttribute("notify", "Pack Restored.");
					rd = request.getRequestDispatcher("restorepack.jsp");
				}
		...
		else if(page.equals("role"))
		{
			try
			{
				...
				if(r >= 1) {
					request.setAttribute("notify", "Role Restored.");
					rd = request.getRequestDispatcher("restorerole.jsp");
				}
		...
		else if(page.equals("user"))
		{
			try
			{
				Class.forName("com.mysql.jdbc.Driver");  
				con=DriverManager.getConnection("jdbc:mysql://localhost:3306/csms",dbuser,dbpass);
				ps = con.prepareStatement("UPDATE user SET deleted=?, status=?, updated_by=? WHERE user_name=? OR mobile_no=? OR email=?");
				ps.setString(1, null);
				ps.setInt(2, 1);
				ps.setInt(3, user);
				...
				if(r >= 1) {
					request.setAttribute("notify", "User Restored.");
					rd = request.getRequestDispatcher("restoreuser.jsp");
				}
